...
**A dispatch is NOT equivalent to a tag push, and treating it as one was the hazard.** Only the version *string* comes from the input — `github.event.inputs.tag || github.ref_name` — and **neither job's checkout takes a `ref:`** (`release.yml:54`, `:201`), so both build `github.sha`, the commit of the ref the run was started from. Without the guard below, **dispatching `tag: v0.4.1` from `main` publishes `main`'s code as `0.4.1`**, signs it, and attests it.

**The guard.** In the `release.yml` that sits beside this page, the image job's `version` step — the step right after checkout, before `push`, `cosign sign`, both attestations and the whole chart job — fails the run when the event is `workflow_dispatch` and `github.ref` is not exactly `refs/tags/<the tag input>`. That test is false on a tag push, so a tag push publishes exactly as before. `make release-workflow` (`test/release/`), in `make test` and in CI, runs that step's own script over a case table, and fails if any later step, or any other job, could run after it refuses, or if a `run:` script inlines any `${{ }}` expression outside a four-entry allowlist. No release has been cut with it yet.

**A dispatch runs the `release.yml` on the ref it was started from**, not the one on `main`. So the guard covers a dispatch only when that ref's `release.yml` contains it. No tag up to and including `v0.4.1` does. For a run whose `release.yml` contains the guard, it refuses:

- a dispatch from a branch, with any tag input — `main` once `main` carries the guard;
- a dispatch from one tag that names another, such as a future `v0.5.0` dispatched as `v0.5.1`;
- a dispatch whose input is not the tag's exact name, such as `0.5.0` for `v0.5.0`. That one used to publish the right code; it is now refused too, because the rule is one string comparison and not a guess.

What the guard does **not** do:

- **It does not protect a run of an older `release.yml`.** A dispatch from any tag up to and including `v0.4.1`, or from any branch whose `release.yml` predates the guard, still builds that ref and publishes it under whatever name you type.
...
- **No `.sig` verification at install time.** Nothing forces a cluster to reject an unsigned assayd chart; that is the Sigstore policy-controller's job (design 07 A2 chose it; A3 defines its enforcement contract) and assayd does not ship one for itself yet. It does not enforce this for agent images either: admission there checks digest-pinning only and verifies no signature, as the correction at the top of this page says.
