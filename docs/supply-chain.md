...
Both are published by `.github/workflows/release.yml`. **Two things trigger it, not one**: a push of a `v*` tag, **and `workflow_dispatch` with a `tag` input** — the version comes from `github.event.inputs.tag || github.ref_name`, so a manual run publishes exactly as a tag push does. This page said "only … on a `v*` tag"; a manual dispatch is how `v0.2.0`'s re-release happened.
