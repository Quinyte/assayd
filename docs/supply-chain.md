...
**A dispatch is NOT equivalent to a tag push, and treating it as one was the hazard.** Only the version *string* comes from the input — `github.event.inputs.tag || github.ref_name` — and **neither job's checkout takes a `ref:`** (`release.yml:54`, `:198`), so both build `github.sha`, the commit of the ref the run was started from. Without the guard below, **dispatching `tag: v0.4.1` from `main` publishes `main`'s code as `0.4.1`**, signs it, and attests it.

**The guard.** In the `release.yml` that sits beside this page, the image job's `version` step — the step right after checkout, before `push`, `cosign sign`, both attestations and the whole chart job — fails the run when the event is `workflow_dispatch` and `github.ref` is not exactly `refs/tags/<the tag input>`. That test is false on a tag push, so a tag push publishes exactly as before. `make release-workflow` (`test/release/`), in `make test` and in CI, runs that step's own script over a case table, and fails if any later step, or the chart job, could run after it refuses. No release has been cut with it yet.
