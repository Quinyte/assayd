...
|---|---|---|
| `ApplyingBackends` | SSA the Backends for the desired set, record digest + generation, return | Backend status watch |
| `ApplyingPolicies` | SSA the Policies, return | Policy status watch |
| `Converging` | evaluate §3.3.2's per-kind tuple over every mandatory resource | status watches; `RequeueAfter` for the deadline |
| `Publishing` | attach or weight the route into traffic | route status watch |
| `Served` | steady state | spec change, or a NACK Event |

`status.apply` carries `{digest, generation, stage, deadline}`. Controller-runtime's per-key serialization is untouched, because each reconcile still returns before the next begins — what is removed is the sleeping.
