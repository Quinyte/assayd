...
- **Status**: **approved** — critique PASS at r2 (reviews/03-review.md; residuals R2-a/R2-b folded in) · ADR-0020. **Amendments A1–A7 (2026-08-25/26), folded into §§3–8. Two independent critiques returned REVISE (round 1: 4/8/7; round 2: 4/12/6); every finding is addressed, and `reviews/03-amendments-review.md` carries both. No pass has returned PASS — these are not yet critique-passed** (§11)
...
**Whether the compiler runs at all is declared, not discovered.** The chart carries `gateway.enabled` (default `true` at core tier, where architecture §17 puts agentgateway). This is the single switch, and it removes the discovery ambiguity that made "the gateway is missing" mean two incompatible things:
...
| `false` | either | **a deliberately ungoverned tier** — what P1 ships and what `local` uses | compile nothing; every Agent carries `GatewayIncompatible=GatewayDisabled`; **`Ready` is not withheld** |

Declared-but-missing is an incident. Declared-off is a documented tier. Only the first withholds `Ready`, so a stock `helm install` never produces a fleet that is permanently un-`Ready` — and an operator who asked for a gateway and did not get one is never told everything is fine. `gateway.enabled: false` is loud rather than silent: the condition is on every Agent, the chart's `NOTES.txt` states that agents are ungoverned, and `plume doctor` reports it (design 08 §7).

**At P1, `gateway.enabled` is `false` and the compiler does not run.** When it is turned on, exactly one route class is emittable — the A2A serving route, and only once design 06 supplies `identity`; until then its mandatory `-auth` input is absent and §3.3.1 makes that a `PolicyCompileFailed`, which is correct and is *not* the "not emitted" branch. Tool, KG and LLM egress routes have no producer at all until designs 11, 01/13 and 06 land, so they are the not-emitted branch. **`llm.egressAllowlist` is therefore not compiled at P1 either** — it rides the LLM egress route — but it is classified behaviour in design 02 A16 today, because gating it is the revision projection's job and does not wait on this compiler.
...
1. **Backends** first.
2. **Policies** next — then **verify acceptance**: poll each policy's status until `Accepted=True`/programmed (**bounded: 30s per resource-set apply, not per policy**, after which the remaining policies are treated as not-accepted and their routes withheld). The poll runs **on the reconcile worker**: controller-runtime's one-reconcile-per-object is the only thing serializing this sequence, and moving the wait off it would let reconcile N's outstanding poll apply old routes after reconcile N+1 had installed new isolation policies — review 03 finding 1, restored. A stuck gateway does not stall the queue, because the bound is finite and the path is re-entrant: withhold the dependent routes, set `PolicyApplyIncomplete`, return, requeue.. A policy that fails webhook or reports not-accepted ⇒ **stop**: dependent routes are not created (or existing ones zero-weighted), condition `PolicyApplyIncomplete` with the resource + reason.
...
**Mandatory sets range over emitted *resources*, not only policies.** An earlier draft scoped them to `AgentgatewayPolicy` kinds, which cannot express the one control ADR-0014 rests on: `llm.egressAllowlist` compiles to a **Backend restriction**, so "the LLM Backend must carry an egress restriction" was outside the stated extension point entirely, and a `hipaa` cluster could emit an LLM egress route with no BAA restriction and nothing in this design would stop it. A mandatory entry therefore names a resource and a predicate over it — `AgentgatewayPolicy(-auth)`, `AgentgatewayBackend(egressRestricted)` — and both are checked before the route is created.
...
The same argument applies to reachability, and an earlier draft made the pricing check and missed it. When `llm.egressAllowlist` is set, **`llm.fallback`'s endpoint must resolve inside it**, checked at compile time on the same footing and failing with `PolicyCompileFailed` naming `spec.llm.fallback`. Otherwise design 20 activates a fallback the gateway's Backend restriction blocks: `ModelDrifted` reports the remediation applied while the agent serves nothing, which is rule 8's loud-and-wrong at the worst possible moment. Design 20 §5 has a row for "fallback provider also drifted" and none for "fallback was never reachable".

With `U`µ and `P`µ as integer micro-USD:

```
tokensPerDay(usd) = floor(Uµ × 1_000_000 / Pµ)
rate              = floor(tokensPerDay / (90_000 × gatewayReplicas))    # tokens/second, the bucket's refill
capacity          = rate × 3_600                                       # burst: one hour's refill
```

**Why 90 000 and not 86 400.** A bucket starts full, so a day's maximum throughput is `capacity + rate × 86 400`. Sizing `rate` against 86 400 and *then* adding an hour's capacity lets through `tokensPerDay × 90000/86400` — **4.17% over the declared budget, per replica** — which is precisely the lateness §3.4 and D3 forbid, introduced by the paragraph that invokes them. Dividing by `86 400 + 3 600` instead makes `capacity + rate × 86 400 = rate × 90 000 ≤ tokensPerDay` exactly, so the burst is paid for out of the budget rather than added to it. The invariant holds by construction and a golden fixture pins it.
...
| Route class whose **producing design is not installed** | Not emitted. No *per-class* condition — one would fire on every Agent in every core-tier cluster and say nothing actionable. The tier gap is reported once at operator level and by `plume doctor` (design 08 §7) |
...
| **Gateway CRDs absent, nothing ever applied** (cold start, A2) | `gateway.enabled: true` and the CRDs are missing, so this is **a broken install, not a tier choice** — the operator asked for governed traffic and did not get it. `GatewayIncompatible=CRDsAbsent`, message **naming the CRD set it checked** (Gateway API `HTTPRoute` *and* `AgentgatewayPolicy`/`AgentgatewayBackend`, which install independently — a cluster running Istio or Cilium has the first and not the others). **Withholds `Ready`**, reports `phase: Pending`, pages. Nothing stands because nothing was ever applied; the row below is the regression branch and does not cover this |
| Gateway CRDs present but version-skewed, config previously applied | `GatewayIncompatible=VersionSkew`; last-applied stands |
| `gateway.enabled: false` — the deliberately ungoverned tier (P1, `local`) | The compiler does not run. Every Agent carries `GatewayIncompatible=GatewayDisabled` and **`Ready` is not withheld**: nothing was promised, so nothing is broken. Loud rather than silent — the condition is per-Agent, the chart's `NOTES.txt` states that agents are ungoverned, and `plume doctor` reports it. This is the row that keeps a stock `helm install` from producing a fleet that is permanently un-`Ready` (§3.1) |
