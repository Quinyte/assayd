...
- **A10 (2026-08-27, ADR-0028 supersedes ADR-0020) — the version was wrong, and two guarantees went with it.** This design was written against "agentgateway 2.2". **There is no 2.x** — releases are `v1.x`, latest stable **v1.4.1**, and `2.2` was a kgateway-era docs path that still returns HTTP 200 while serving content predating `AgentgatewayModel`, `virtualModel` and `oauthTokenExchange`. Every citation resolved and served stale content, which is why four internal critique rounds could not reach it and a cross-family review plus a primary-source re-verification did.
  ...
  **Not fixed here, and named**: `egressAllowlist` still compiles to a "Backend restriction" in §3.3.1/§3.4 — the CRD has **no egress or allowlist field anywhere**, so that is Backend *construction* plus a plume-owned provider→endpoint catalog and a `dynamicForwardProxy` ban, which is a design change rather than a wording fix. The MCP tool filter still describes `Mcp-Name` header matching; it is now first-class CEL at `spec.backend.mcp.authorization`, which filters `tools/list` items rather than only rejecting calls. Both are owed. Research §9's gaps — evaluation order, negative-burst runtime behaviour, whether a NACK'd policy fails open — **may not be encoded**; the last one blocks the update-ordering design above.