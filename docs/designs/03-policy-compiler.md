...
|---|---|---|
| `Quiescing` | *update transactions only* — strip `backendRefs` from the live route so it stops serving the old rule | route status watch |
...
| `Witnessing` | restore `backendRefs` behind a match admitting **only the probe identity**, and require the witness (§3.3.3) | probe result |
| `Publishing` | widen the match to real traffic | route status watch |
...
### 3.3.3 Transaction kinds, and the witness (A17)

Three kinds, persisted in `status.apply.transaction`, because the stages and the risks differ:

| Kind | When | Stages | Traffic cost |
|---|---|---|---|
| `Create` | no route is serving yet | `PreparingRoute` → Backends → Policies → `Converging` → `Witnessing` → `Publishing` | none — nothing was serving |
| `Tighten` | any mandatory concern becomes stricter | `Quiescing` → the above | **the route returns 500 for the whole transaction** |
| `Loosen` | the new set is provably no stricter than the old | Backends → Policies → `Converging` → `Publishing`, route stays live | none |

**Anything not provably a loosening is a `Tighten`.** The classification is fail-closed by default: a set is `Loosen` only when every mandatory concern's admitted set is a superset of the old one and no concern is newly required. Anything else — including a change the compiler cannot classify — quiesces. Being wrong in the `Tighten` direction costs availability; being wrong in the `Loosen` direction serves the old permissive rule, which is the defect this section exists to remove.

**The 500 is the honest cost and must be documented, not discovered.** §2.6 measured the inert route: attachable, `ResolvedRefs=True`, and it answers **HTTP 500**. A tightening therefore takes the route down for its convergence window rather than serving the previous rule. That is the correct direction — a 500 leaks nothing — but an operator who is not told will read it as an outage, so `PolicyApplyIncomplete` is not the condition here: the CLI and status name the transaction and its stage.

**The witness is positive evidence, and there is no substitute.** No upstream ACK exists (§3.3.2), so the only sound proof that the *new* rule is enforcing is to observe it enforcing. `Witnessing` restores `backendRefs` behind a match admitting **only the probe identity** — the same header-matched, identity-admitted shape design 02 §3.3 already uses for candidate routes — and requires a probe that the new rule must reject:

| Concern | Witness |
|---|---|
| `-auth` newly required | an unauthenticated probe must receive 401/403 |
| `-toolfilter` narrowed | `tools/list` must not contain the removed tool, and calling it must be rejected |
| egress narrowed | a probe to a removed destination must fail to connect |
| `-ratelimit` lowered | probes beyond the new rate must receive 429 |

Attribution is sound because the witness runs on **the same route and the same policy attachment** that will carry production traffic — not a copy — and because the route admitted nothing else during `Quiescing`. `Publishing` then only **widens the match**; it changes no policy, so it cannot re-open the barrier. That is a smaller risk than a fresh apply, not zero, and it is stated rather than assumed.

**A concern with no witness cannot be tightened online.** If a mandatory concern has no probe whose outcome is attributable only to the new set, the transaction is refused and the change must go through the revision path instead — mint a candidate, gate it, shift weight (design 02 §3.3). Publishing on the absence of an error is what §2.8 measured as fail-open, and this design will not do it.
