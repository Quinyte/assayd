...
- **A1 (2026-09-03, from design 26 A1 and the cross-family review of design 02 A60) — a profile on a hard-mode tenant reports gate integrity as an attested exception, never as passing.** In hard mode the tenant's own vCluster admin can replace a gated revision's material (design 26 A1: `GatesTenantAdminBypassable=True`, always, in that mode). §3's posture contract turns every safeguard row into an admission rule and a `ComplianceViolation` on drift; a hard-mode tenant with the HIPAA pack would otherwise report every row passing while its admin swaps material under a gated digest. So the profile **consumes the condition**: on a Tenant carrying `GatesTenantAdminBypassable=True`, the rows that rest on ADR-0006's gate — access controls via compiled policy, model-side egress constraint, audit completeness of what a gated revision serves — are reported as **attested exceptions** in the shipped posture document, naming the tenant's admin as the attesting party, and are never reported as conforming. A profile that cannot accept an attested exception refuses the tenant. **Owed**: the posture fixture for a hard-mode tenant.
