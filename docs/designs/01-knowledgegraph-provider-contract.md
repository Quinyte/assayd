...
- `kg.probe` is in the query surface so readiness, `assayd kg probe`, and conformance share one path — but gateway policy scopes it to platform identities (agents never call it).
...
`assayd-kgp-conformance` (container + `assayd kgp conformance --endpoint …`): runs a fixture ontology ("clinic-sop", ~40 nodes) + seeded corpus against any endpoint:
...
- **A1 (2026-08-20, from design 03 r1 finding 6)**: KG scope enforcement mechanism made precise: the gateway **injects** a signed `X-Assayd-KG-Scope` header (it does not parse MCP bodies — consistent with §3.1) — **the word "signed" is wrong; see A8 below**; **providers enforce scope across all four fact-bearing tools** (`search`, `neighbors`, `get_context_bundle`, `cite`) answering `KG_SCOPE_DENIED` for out-of-scope access. Consequence: providers are scope-enforcing, and the **conformance suite gains a scope-enforcement battery** (out-of-scope search/traversal/bundle/cite must deny). §6's "enforced at the gateway" reads as "gateway-injected, provider-enforced".
...
- **A8 (2026-08-25, from design 03 A5)**: A1 above (and ADR-0020 decision (5)) says the gateway injects a **signed** `X-Assayd-KG-Scope` header. That word is wrong and A1 is left standing so the correction is visible rather than silent. **No header signature exists or is needed**: ingress to a provider is gateway-only over mutual TLS (§6), so trust derives from the gateway's verified SVID on that connection — design 13 §3.3 states this and records that design 03's wording was aligned to it. Nothing else in A1 changes: the gateway still injects rather than parses, providers still enforce across all four fact-bearing tools, and the conformance scope-enforcement battery still applies. The consequence of the correction is that no signing key, key distribution or verification step is owed by any design — an implementer reading A1 alone would have built all three.
