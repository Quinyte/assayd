...
- **A6 (2026-08-31, from an independent critique of design 02 A36–A43)**: three gaps in A5's verification, all in the direction of claiming more than was measured. **The witness was unbounded**: one receipt proves one gateway replica took the new weights, and design 03 §3.1 declares `gatewayReplicas` may exceed one, so the other N−1 could still be routing production traffic to the drifted primary while plume reported the dataplane converged — the receipt was to carry `hop.gatewayInstance` (design 04 A4) and a partial result to be named as such. **A7 withdraws that requirement**: design 04 names no producer for `hop.gatewayInstance` — nor for `hop.endpoint`, which A5's canary attributes by — so neither check has an input, and a per-replica count would measure load balancing rather than coverage even once they do. **The canary was not admitted**: A5 required it to run through the *production* route, whose admitted set is the agent's SVID; adding the `drift:model-canary` principal at activation would widen a serving route's admitted set mid-incident, which is a behaviour-surface change. It is admitted at revision publish instead, like design 16's eval principal. **The reverse shift had no postcondition at all** — a shift back that did not land clears `llmFallbackActive` while traffic still goes to the fallback, typically a cheaper or weaker model, indefinitely and after the incident when nobody is watching. A5's own argument applied unchanged and had not been applied.

- **A7 (2026-09-04, from Codex r8 BLOCKER 13 via design 03 A50) — A6's replica-complete witness is withdrawn, and the canary cannot attribute its result at all yet.** A6 required `hop.gatewayInstance` to distinguish "the dataplane converged" from "one of N instances did", and design 03 A43 turned that into a per-replica requirement before `llmFallbackActive` could go true. Checked against the producing design: **design 04 names no producer for `hop.gatewayInstance` and none for `hop.endpoint` either** — A4 adds both fields with a rationale and no attribute, no stamping mechanism and no grammar (design 04 A5). So A6's witness has no input, and A5's canary, which attributes by the full endpoint identity in the receipt, has none either: `llmFallbackActive` cannot legitimately go true on any cluster today, one replica or many.

  The per-replica requirement is withdrawn beyond that, because it would not be a proof even once the fields exist: nothing routes a canary to a *chosen* replica through a Service, so a count of distinct instances measures load balancing rather than coverage, and a rollout leaves stale instance identities in recent receipts that inflate the count while a current replica goes unwitnessed. When the producers land, the claim is bounded — one replica gives a decidable proof; more than one gives evidence about one instance, `LLMFallbackUnavailable` names the instances seen and the number expected, and the remediation is not recorded as clean. A per-replica proof remains possible in principle by resolving the gateway's `EndpointSlice` and dialling each Pod directly, which is a dataplane-probing mechanism neither this design nor design 03 has, and which would need its own decision.
