...
Dashboards (OpenObserve JSON, chart-shipped, name-mapped): **Fleet** (agents × phase/eval/cost/success), **Agent drill-down** (signals + recent receipts link), **Task trace** (gateway + interior spans joined), **Platform health** (operator, tap, gateway, SPIRE, IdP probes), **Budget** (spend vs limits, PricingStale flags). Alert rules (shipped, tunable): success-rate SLO burn (page), `ReceiptsDegraded` (page), candidate held >1h (ticket), tool-error spike (ticket), `termination_reason != goal_met` rising (ticket), cert/SVID issuance failures (page), backstop staleness >60s (ticket), `PolicyApplyIncomplete` open >5m (page — a route is withheld, so traffic is being refused), `PolicyCompileFailed` (ticket), `GatewayIncompatible=CRDsAbsent` (page — a declared gateway is missing; keyed on the **reason**, at every profile, since `VersionSkew` is stale-but-serving and the declared-off tier is `GovernanceSkipped`, which is never an alert), `BudgetEnforcementDegraded=ReplicaSkew` (ticket — the divisor drifted, limits are wrong by a known ratio). Every alert names the condition/design it maps to — alerts and CR conditions tell one story.
