...
The lease is therefore named for something knowable **before** any content is read — `plume.dev/src.<agent-uid8>.<leaseUID>`, where `leaseUID` is durable, random, and recorded on the Agent *before* the source is touched. The order is: **record the lease UID → add and observe the finalizer → read and hash the sealed source → associate the lease UID with the resulting revision hash in status.** A crash at any step leaves either an unassociated lease, which the startup sweep reclaims, or an associated one, which is live; never a source sealed by a name nothing can resolve. The finalizer is never renamed, so there is no add-then-remove window to get wrong.
...
**The digest in status is keyed, because a raw one is a guessing oracle (A27).** `status` is readable by anyone with `get` on Agents, which is a far wider set than those who can read the Secret. A raw `sha256` of a low-entropy value — a feature flag, a username, a short token, `true`/`false` — is trivially reversed offline by anyone holding it. So the digest published in status is `HMAC(cluster-key, content)`, with the key in an operator-only Secret; equality comparison still works, offline guessing does not. Rotating the key re-derives every retained digest in one pass and is a normal operation, not a migration.
