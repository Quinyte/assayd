...
- **Status**: **integrated r6** — amendments A1–A34 folded into the body, all critique-driven and **none critique-passed**: every round has returned REVISE. Latest: `reviews/03-codex-review-r6.md` — A30–A34 address its BLOCKER 2 and MAJORs 2, 3 and MINOR 1, and record **BLOCKER 1 (unplanned guard loss) as an OPEN race with no chosen remedy**. A30 is also a hash **migration** (`internal/revision/golden_test.go`, MIGRATION 1) (§12) (the independent re-critique asked for one integration pass rather than a rule telling implementers which half to believe). Amendment history is preserved in §12 for provenance only; **the body is authoritative**. · ADR-0019
...
1. Spec change → `revisionHash(projection)` — a projection of the behaviour surface **plus the resolved contents of every env source it references** (A20); the card digest is status, and card drift triggers re-registration, never a new revision → creates a parallel workload `<name>-<hash>`.
   ...
   **Hashing detects a change; it does not stop the serving revision consuming one (A23).** Content hashing mints a *candidate* when a source changes, but the retained active workload still references the object **by name**, so a node drain recreates an R1 Pod that reads the new content under R1's gated hash and route — no Agent write required. The digest recorded in status protects nothing by itself. So every referenced source is **sealed** for as long as a retained revision holds it: a `plume.dev/env-source-protection` finalizer plus a chart-shipped, fail-closed `ValidatingAdmissionPolicy` denying delete, label removal and finalizer removal by every principal except the operator — and denying **`data`/`binaryData` change by everyone, the operator included** (A27). **No bytes are copied.**
...
- **Unplanned guard loss cannot be handled by reacting, and "fall back to snapshots" was not a fix (A34).** The sentence that stood here made A23's snapshot fallback the thing that closes this race. It cannot be, because it is *reactive*: at the moment of loss no snapshot exists, no PodSpec references one, and once the editor has won, the original bytes are gone. A fallback triggered afterwards can only copy the attacker's new content or fail for want of the old. A remedy that runs after the loss cannot restore a pre-loss invariant.
  ...
  **This race is OPEN.** Closing it needs an invariant that holds *before* the guard is lost, and there are two candidates:

  | Candidate | What it costs |
  |---|---|
  | **Immutable revision-scoped material, created before publication**, with the revision's PodSpec referencing it | the copying A23 declined: a retention horizon that archives historical credentials, an API/backup/audit/RBAC/GC surface per object, and a rollback that can resurrect a retired credential. The security, GC and rollback contract is already analysed in `reviews/03-codex-review-b3.md` |
  | **A separately protected Pod admission control** that refuses a replacement Pod unless every source it references is guarded and matches the revision's recorded material | a second admission path whose own removal must be detectable, and a `failurePolicy: Fail` blast radius that must be scoped to plume-managed namespaces or it stops unrelated Pods when the webhook is down |

  Neither is chosen here. Until one is, **the seal is not fail-closed against unplanned guard loss** and this design does not claim it is.
