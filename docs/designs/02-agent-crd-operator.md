...
- **Status**: **integrated r3** — amendments A1–A17 folded into the body. A15–A17 (2026-08-25/26) are **not yet critique-passed**: two rounds returned REVISE and all findings are addressed, but no pass has returned PASS. A18–A19 are owed and undrafted — each needs a decision, not a correction (§12) (the independent re-critique asked for one integration pass rather than a rule telling implementers which half to believe). Amendment history is preserved in §12 for provenance only; **the body is authoritative**. · ADR-0019
...
spec:
  ...
  budget: {tokensPerDay: 2000000, usdPerDay: "40.00", taskTimeout: 10m, maxHops: 8}
                                                  ...
                                                  # usdPerDay is a decimal STRING: ^[0-9]{1,7}(\.[0-9]{1,6})?$ (A15)
...
1. Spec change → `revisionHash(spec)` — **spec only**; the card digest is status, and card drift triggers re-registration, never a new revision → creates a parallel workload `<name>-<hash>`.
   ...
   **The hash covers a projection of spec, not all of it (A12, r2).** Two surfaces share one CR:
   ...
   4. **`llm.egressAllowlist` is behaviour, and was in neither column (A16).** It is the set of endpoints an agent may reach, so widening it changes what the agent can do — the same argument A12 already makes for `knowledge[].scope`, where narrowing what an agent may retrieve changes what it answers exactly as removing a tool does. It is also the control ADR-0014's compliance profiles pin, so an ungated edit would let an operator widen egress on a HIPAA cluster with no eval and no gate. Design 03 §3.1 consumes it, which is how the omission surfaced.

   **The gate is asymmetric, because the risk is asymmetric.** ADR-0006's gate exists to stop capability *expansion* reaching production unevaluated. Adding an endpoint expands capability and **mints a revision**. Removing one can only reduce it, and is **applied in place**. Without that split the urgent direction is the slow one: an endpoint loses BAA coverage, and revoking it would queue behind an eval-and-canary cycle on every agent — design 27 §3 enforces a profile by rejecting non-conforming Agent CRs, so a BAA list change is applied by editing every Agent, and on 200 agents symmetric gating means 200 candidates Held at weight 0 while a compliance violation stays live. Rules 4 and 5 below reason exactly this way about incident response, and design 20's status-shaped fallback exemption is the same trade. Compared as a **set**, not a list: the projection already sorts `llm.providers`.
...
So: a **type-keyed perturber registry**, and it **fails closed**. Types with unexported state (`resource.Quantity`, `metav1.Time`, `metav1.Duration`, `intstr.IntOrString`) get an explicit perturber that changes their *semantic value* through their public API. A type appearing in the graph with no registered perturber **fails the test**; it is never skipped. Enumerating *types* is safe where enumerating *paths* is not: the type set is small, stable, and a new one announces itself as a failure rather than a silent gap.

**Map-kinded leaves are defined too.** `ResourceRequirements.Limits/Requests` are maps, and "the transitive leaf graph" has no leaf identity for a map. A map-kinded leaf is perturbed twice — once over its key set, once over its value set — and either mutation must mint if the leaf is behaviour. It is moot today because `runtime.resources` is policy and the walk prunes there, but the rule is stated generally and the type is already in the graph.
...
| **Gateway CRDs absent** (A15) | `gateway.enabled: true` is a **broken install**: `GatewayIncompatible=CRDsAbsent`, `Ready=False`, `phase: Pending`, page — at every profile, `local` included. `gateway.enabled: false` is a **declared tier**: `GatewayIncompatible=GatewayDisabled`, `Ready` unaffected. Nothing stands either way, because nothing was ever applied |
...
- **`gateway.enabled: false`** — a deliberately ungoverned tier, which is what P1 and `local` ship. `GatewayIncompatible=GatewayDisabled`, **`Ready` is not withheld**: nothing was promised, so nothing is broken. Loud, not silent — the condition is on every Agent and the chart's `NOTES.txt` says agents are ungoverned.
...
**`llm.egressAllowlist`'s gate is asymmetric (MAJOR 7).** Classifying it behaviour was right; gating both directions was not. Design 27 applies a profile by rejecting non-conforming Agent CRs, so a BAA list change is applied by editing every Agent — and symmetric gating would put 200 candidates in `Held` while a live compliance violation waited on eval cycles. Adding an endpoint expands capability and mints a revision; removing one cannot, and applies in place. ADR-0006's gate exists to stop expansion, and this is the same reasoning rules 4 and 5 already use for incident response.

**`Ready` is rekeyed on `gateway.enabled` (BLOCKER 3)**, retracting both of A15's earlier framings — see A15 above for why profile was the wrong key and what the declared-configuration split is. **ADR-0027's erratum was itself false** and is corrected there: the test asserts a hard-coded 21 while §3.1 declares 25, so nothing is enforced, not even the count.

**Still owed, unchanged**: A18–A19 (`envFrom` ConfigMap contents; mutable image tags), each needing a decision rather than a correction; `gateway.enabled` in the chart and the operator; the `designConditions()`-vs-§3.1 comparison; and the four condition constants, which do not exist in code.
