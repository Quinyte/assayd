...
- **Status**: **integrated r3** — amendments A1–A16 folded into the body. A15–A16 (2026-08-25) are **not yet critique-passed**; A15 was revised after critique returned REVISE and A16 has had none. A17–A18 are owed and undrafted — each needs a decision, not a correction (§12) (the independent re-critique asked for one integration pass rather than a rule telling implementers which half to believe). Amendment history is preserved in §12 for provenance only; **the body is authoritative**. · ADR-0019
...
1. Spec change → `revisionHash(spec)` — **spec only**; the card digest is status, and card drift triggers re-registration, never a new revision → creates a parallel workload `<name>-<hash>`.
   ...
   **Classification is compulsory, not defaulted, and it is LEAF-level (A16).** Every `AgentSpec` field is named in exactly one column, and `TestEveryFieldIsClassified` reflects over the struct to prove the table is exhaustive: adding a CRD field fails the build until someone classifies it and amends this table.
   ...
   So the rule is: **classification and projection are over the transitive leaf graph of `AgentSpec`, not its top-level fields.** A table row may still name an aggregate (`runtime.env`), but the aggregate's classification binds every leaf beneath it, and the test must reach every leaf to prove it.

   **The test must reflect, never enumerate.** These are `corev1` structs that gain fields between Kubernetes releases, and this exact trap has already been sprung once: A12 rule 2 records that `envSourceRef` named four arms while k8s v0.36.4 had five, so `fileKeyRef` collapsed to a constant and repointing `prod.env → staging.env` minted no revision. A hand-maintained leaf list reintroduces that failure on the next dependency bump. The test walks the live type graph and fails on any leaf whose perturbation leaves the hash unchanged unless that leaf is explicitly classified policy.
...
| **Gateway CRDs never installed** (A15) | `GatewayIncompatible=CRDsAbsent`. Nothing stands — nothing was ever applied. Outside the `local` profile this withholds `Ready` and reports `phase: Pending`; inside it, the condition is raised and does not withhold (A15 — the carve-out stands on the developer loop, not on the `GatesBypassed` analogy an earlier draft cited) |
...
**What the closure test actually enforces, stated because the previous sentence was false.** `api/v1alpha1/schema_contract_test.go` asserts only that the list has the declared length and no duplicates — it never compares names to anything. A critic mutation-checked it: renaming `CondPricingStale` to `TotallyWrongName` **survived**. So "the vocabulary is closed and mechanically pinned" was true of the cardinality and nothing else, and ADR-0027's claim that this rule was verified by mutation was wrong. The implementation change this amendment owes is to compare `designConditions()` against the list parsed from §3.1 above, the way the CRD golden already works. Until that lands, the enforced bound is the count.

**`Ready` under a gateway that was never installed.** §5 described gateway trouble only as a regression — "last-applied stands" — which is vacuous when nothing was ever applied, and that is the only state a cluster without the agentgateway subchart is ever in. An agent whose budget, authn and tool-filter were never compiled is not serving under the guarantees `Ready` asserts: architecture §02 puts every guarantee at a hop the traffic never took. So `GatewayIncompatible=CRDsAbsent` **withholds `Ready`**, and the agent reports **`phase: Pending`** — not `Degraded`, which would hold design 20's retention pin open forever on a gateway-less cluster and let revisions accumulate without bound.
...
**An earlier draft justified the `local` carve-out by analogy to `GatesBypassed=DevProfile`, and that analogy was wrong.** `GatesBypassed` is *opt-in* — design 08 §5 reaches it by explicit profile flag on dev revisions. `GatesSkipped` is the environmental one, and `CRDsAbsent` is environmental too, so the honest analogue is `GatesSkipped`, which does not vary by profile and does not withhold `Ready`. The carve-out therefore stands on its own merits or not at all: **in `local`, the condition is raised and does not withhold**, because the developer loop is the only thing running on a laptop cluster and blocking it buys no safety — nothing is serving production traffic to protect. That is a weaker argument than the one it replaces, and it is the true one.

**Two things this owes, both unimplemented and named so they are not assumed.** The operator has no profile input — `cmd/operator/main.go` declares four flags and none is `--profile`, and the chart passes none, so the rule above cannot run until `.Values.profile` is plumbed the same way `--eval-suite-check-interval` is. (Its cited precedent `GatesBypassed` is likewise declared and set nowhere.) And the carve-out **expires** when design 07's chart ships agentgateway at core tier: from then on `CRDsAbsent` means a broken cluster at every profile including `local`, and the carve-out becomes wrong with nothing scheduled to remove it unless this is written down.
