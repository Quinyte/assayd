...
- **Status**: **integrated r3** — amendments A1–A18 folded into the body. A15–A18 (2026-08-25/26) are **not yet critique-passed**: three rounds returned REVISE and all findings are addressed, but no pass has returned PASS. A19–A20 are owed and undrafted — each needs a decision, not a correction (§12) (the independent re-critique asked for one integration pass rather than a rule telling implementers which half to believe). Amendment history is preserved in §12 for provenance only; **the body is authoritative**. · ADR-0019
...
**Still owed, unchanged**: A19–A20 (`envFrom` ConfigMap contents; mutable image tags), each needing a decision rather than a correction; `gateway.enabled` in the chart and the operator; the `designConditions()`-vs-§3.1 comparison; and the four condition constants, which do not exist in code.
...
**Tier-absence becomes `GovernanceSkipped`, its own condition type (BLOCKER 2).** Condition count **26**. See design 03 A8.
