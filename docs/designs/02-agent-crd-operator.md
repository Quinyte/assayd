...
- **Status**: **integrated r7** — amendments A1–A37 folded into the body, all critique-driven and **none critique-passed**: every round has returned REVISE. Latest: `reviews/03-codex-review-r6.md` — A30–A34 address its BLOCKER 2 and MAJORs 2, 3 and MINOR 1, **A35 closes its BLOCKER 1** with user-chosen immutable revision-scoped copies, superseding A23's seal (the retirement sweep of the seal machinery is owed). A30 is also a hash **migration** (`internal/revision/golden_test.go`, MIGRATION 1) (§12) (the independent re-critique asked for one integration pass rather than a rule telling implementers which half to believe). Amendment history is preserved in §12 for provenance only; **the body is authoritative**. · ADR-0019
...
1. Spec change → `revisionHash(projection)` — a projection of the behaviour surface **plus the resolved contents of every env source it references** (A20); the card digest is status, and card drift triggers re-registration, never a new revision → creates a parallel workload `<name>-<hash>`.
   ...
   **A revision reads its own copy, and the copy is what is hashed (A35).** At revision mint, **before publication**, the operator copies every referenced `ConfigMap` and `Secret` into revision-scoped material, and the revision's PodSpec references only the copies. The user's object is never referenced by a published revision, so editing it, deleting it, or losing every guard that protected it cannot change what that revision serves. A change to the original is *detected* — it mints a candidate, as A20 says — and it reaches production only through a gate.
   ...
   | Ownership / GC | `ownerReference` to the Agent, so copies never outlive it; and copies for a revision are deleted in the same pass that retires the revision. With retention at three live revisions (active, candidate, rollback target), the bound is **3 × sources-per-Agent** |
