...
- **Status**: **integrated r6** — amendments A1–A35 folded into the body, all critique-driven and **none critique-passed**: every round has returned REVISE. Latest: `reviews/03-codex-review-r6.md` — A30–A34 address its BLOCKER 2 and MAJORs 2, 3 and MINOR 1, **A35 closes its BLOCKER 1** with user-chosen immutable revision-scoped copies, superseding A23's seal (the retirement sweep of the seal machinery is owed). A30 is also a hash **migration** (`internal/revision/golden_test.go`, MIGRATION 1) (§12) (the independent re-critique asked for one integration pass rather than a rule telling implementers which half to believe). Amendment history is preserved in §12 for provenance only; **the body is authoritative**. · ADR-0019
