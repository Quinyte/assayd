...
1. Spec change → `revisionHash(spec)` — **spec only**; the card digest is status, and card drift triggers re-registration, never a new revision → creates a parallel workload `<name>-<hash>`.
   ...
   **This retracts the credential-rotation exemption, and that is a real cost.** An earlier version of this rule said rotating a value inside a Secret is not a behaviour change "which is correct". Kubernetes has no notion that a Secret holds credentials rather than behaviour — `envFrom.secretRef` carries a system prompt exactly as well as an API key — so exempting the Kind exempted the bypass. Under content hashing, **a credential rotation now mints a revision and fires a gate.** That is the price of closing it, and it is stated here rather than discovered in production. Any union arm the implementation does not name explicitly must hash **injectively** (over-gate), never collapse to a constant — a constant let `fileKeyRef` repoints through ungated.
