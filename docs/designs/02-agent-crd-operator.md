...
- **Status**: **integrated r7** — amendments A1–A40 folded into the body, all critique-driven and **none critique-passed**: every round has returned REVISE. Latest: `reviews/03-codex-review-r6.md` — A30–A34 address its BLOCKER 2 and MAJORs 2, 3 and MINOR 1, **A35 closes its BLOCKER 1** with user-chosen immutable revision-scoped copies, superseding A23's seal (the retirement sweep of the seal machinery is owed). A30 is also a hash **migration** (`internal/revision/golden_test.go`, MIGRATION 1) (§12) (the independent re-critique asked for one integration pass rather than a rule telling implementers which half to believe). Amendment history is preserved in §12 for provenance only; **the body is authoritative**. · ADR-0019
...
So the name and the identity are separated. `status.activeRevisionDigest` and `status.candidateRevisionDigest` carry the **full** digest and are the only values compared to decide whether two specs are the same revision; the ten-character form stays for object names and the `ACTIVE` column, where a 64-character string is unreadable and a name is all that is needed. A workload carrying a `plume.dev/revision-digest` annotation that disagrees with the desired digest is a **`RevisionHashCollision`**: terminal, no adoption and no rewrite, because converging it is precisely the payload the collision buys. An **absent** annotation is not a mismatch — a workload predating the field is adopted and stamped, or every upgraded cluster wedges.
...
1. Spec change → `revisionHash(projection)` — a projection of the behaviour surface **plus the resolved contents of every env source it references** (A20); the card digest is status, and card drift triggers re-registration, never a new revision → creates a parallel workload `<name>-<hash>`.
   ...
   **One read, one buffer, one publish (A39).** The rules above are only implementable if a single GET produces one immutable byte buffer that is used for BOTH the hash and the Create. The natural two-read implementation — hash the original, read it again to build the copy — races an editor between the reads: the first GET reads safe bytes and computes H, the editor writes malicious bytes, and the operator creates `<agent>-H-env-0` containing the malicious content under a name and a gate identity that say H. The copy would then be faithfully immutable and faithfully wrong.
   ...
   `AlreadyExists` on step 4 is **not** an adoption. A copy named for this revision that this operator did not just create must satisfy the whole invariant — same kind, same owner UID, immutable, byte-identical — or it is `RevisionMaterialCollision` and the revision is not published. Adopting by name is how a name that looks right comes to hold content nobody hashed.
...
The operator normalizes **one revision's** material (tools, budget, `llm`, expose, KG scopes, that revision's weight, `gatewayReplicas`) — one `PolicyIntent` **per live revision**, never one per Agent (design 03 A26) — into a `PolicyIntent` and calls the **policy compiler as an in-process library**, which emits agentgateway resources under fail-closed apply ordering; the operator owns their lifecycle (ownerRefs on the host cluster). The gateway CRD surface is pinned by the chart (design 07) at a minimum version carrying OSS token exchange and virtual models.
