...
**The identity is not truncated (A29).** An earlier draft used `src.<agent-uid8>.<revision8>` — 64 bits across two truncated halves, in a list Kubernetes treats as a **set**. Two holders whose truncations collide share one string, so the first release removes the second's hold and unseals a source a live revision still references, silently. `leaseUID` is a full 128-bit value rendered to fit the qualified-name segment limit (253 characters, so length is not the constraint), and the Agent records the mapping. A `leaseUID` already present on a source when this Agent did not put it there is a **collision that is refused and reported**, never coalesced — a lease is per-holder or it is not a refcount.

**Acquisition order, and why the sweep cannot race it.** The order is: **record the lease UID in Agent status → add and observe the finalizer → read and hash the sealed source → associate the lease UID with the resulting revision hash.** The recorded UID *precedes* the source write, so it is the durable acquisition intent.
...
Each recorded digest therefore carries the **key ID** that produced it, and rotation is: publish the new key → re-derive and write digests under the new ID, revision by revision, each write independent → only when none remain under the old ID, retire it. Verification accepts **any retained key**, so a half-finished rotation compares correctly in both directions and can be interrupted, resumed, or abandoned.
...
**"The exposure is prospective" was false** — see §3.3's guard-loss table. A finalizer stops a delete, not a write, so Binding-gone plus a legitimate editor plus a node drain serves changed content under a gated hash *before* the watch fires. Planned guard changes are overlap-only; unplanned loss falls back to snapshots, which makes A23's fallback the thing that closes this race rather than a contingency for the refcount.
