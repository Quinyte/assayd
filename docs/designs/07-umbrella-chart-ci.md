...
| `gateways` (`gateway.networking.k8s.io`): `get`, on `gateway.name` only | design 03 A75 reads the assayd Gateway's labels, listeners and `allowedListeners` before a `401` is credited, and holds when it cannot | a Role and RoleBinding, `-gateway-labels`, in `gateway.namespace` with `resourceNames: [gateway.name]`, rendered only with `gateway.enabled`. Nothing lists or watches Gateways, and nothing reads a ListenerSet. *(Added by design 03 A75.)* |
...
**Hostname and port, and why rule 3 exists.** Gateway API hands a request to a listener by port, then by the most specific listener hostname that matches, and only then to the routes attached to that listener. The harness's listeners are on separate ports, `http` on 8080 and `tools` on 8081, so there a tool route cannot receive a request sent to the serving port. A Gateway can, though, carry a second listener on the serving port with a hostname such as `*.assayd.internal`. A request for `pricer.payments.assayd.internal` would then go to that listener, and a tool route there matching the host would take the Agent's traffic, with every caller's API key. It could also answer the policy compiler's anonymous probe with a `401` that is not `<agent>-auth`'s (design 03 §3.3.3). This is the capture set design 03 A75's `capturingListeners` reasons about for policies: a listener on the serving port whose hostname is unset or matches the host. **That capture is argued from Gateway API's listener selection, not measured on agentgateway 1.5.0.** The harness's listeners are on separate ports, so no test here puts two listeners on one. Rule 3 refuses the hosts either way.

The chart cannot read the Gateway's listeners. It renders no Gateway (A1), and a `paramRef` to the Gateway would, while the Gateway is absent, deny every HTTPRoute write in the cluster or skip the policy (A5.9's reason for rendering identities inline). So rule 3 refuses by host instead. The serving hosts are `<agent>.<namespace>.<suffix>`, where the suffix is `gateway.hostnameSuffix`, or the operator's default `assayd.internal` when the value is empty, as `operator.yaml` falls back. The chart refuses to render, and the operator refuses to start with, a suffix that is not a lowercase DNS name, so the rule and the routes compare the same string. A tool route writer's hostnames must all be clear of it. These are refused:
...
**The review of PR #39 returned APPROVE**, with no blocker, no major and six minors, each fixed:

- **A tool route in a run namespace reaches an Agent with no `<agent>-auth`.** It is recorded under *What stays open*, and `docs/install.md` §6 tells administrators how to keep it shut.
- **ListenerSets.** `docs/install.md` §6.1 now says not to set `spec.allowedListeners`. The reviewer measured an identity in no list admitted with a `parentRef` of kind `ListenerSet`.
- **The hostname suffix's case.** The chart lower-cased `gateway.hostnameSuffix` for rule 3 and the operator did not, and the operator checked only that it was non-empty. So an uppercase suffix made the two compare different strings, and every emitted route failed the HTTPRoute CRD's hostname pattern. This was reproduced first against a real API server: `pricer.<ns>.Agents.Example` was refused (`spec.hostnames[0] ... should match '^(\*\.)?[a-z0-9]...'`), and `NewAgentReconciler` accepted the suffix. Now the chart fails at render (`assayd.validate`), and the operator refuses at startup any suffix that is not a lowercase DNS subdomain, with an error naming the flag and a lowercase example. Rule 3 uses the value as given. The chart test that pinned `Agents.Example` as rendering now pins a lowercase suffix.
- **A group every identity carries**, such as `system:authenticated`, turns `toolRouteWriters` or `apiKeyWriters` off. `values.yaml` and `docs/install.md` now say so.
- **Design 03 §3.2, §6 and A76 said only the operator's ServiceAccount writes on `http`.** `admission.extraOperators` does too, and they say so now.
- **The shared-port capture** is marked as argued from the specification, not measured on agentgateway 1.5.0.

| Mutation | Killed by |
|---|---|
| MA `assayd.validate` accepts any suffix | `TestANonLowercaseHostnameSuffixDoesNotRender`: all four bad suffixes render (`hostname_suffix_test.go:27`) |
| MB the operator accepts any suffix | `TestAnEnabledGatewayWithNoGatewayNamedIsRefused`: the uppercase and trailing-dot cases build (`httproute_test.go:335`) |
| MC the operator refuses every suffix | the same test's control: a lowercase suffix is refused (`httproute_test.go:348`) |
...
- **A tool route in a run namespace reaches an Agent with no `<agent>-auth`.** Suppose a tool route writer also holds `httproutes` RBAC in a run namespace, and a tool listener's `allowedRoutes` selects that namespace, by `from: All` or by a selector that matches it. The writer can create a plain HTTPRoute on that listener, with a hostname rule 3 admits, whose `backendRefs` name an Agent's revision Service in the same namespace. No Backend and no `ReferenceGrant` is involved, and the Agent is reached with no `<agent>-auth` in front of it. Nothing in admission refuses it: the rule reads a route's `parentRefs` and `hostnames`, not its `backendRefs` or its namespace. The review of PR #39 found it. The defence today is the administrator's, and `docs/install.md` §6 states it: never grant a tool route writer route RBAC in a run namespace, and never let a tool listener's `allowedRoutes` select one. Refusing a tool route writer's routes in namespaces labelled `assayd.dev/run-namespace: "true"`, a label A5.9 reserves to the operator, would close it in admission. That is not done.
