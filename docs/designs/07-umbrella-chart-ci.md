...
- **Stateful deps**: renders Postgres + NATS only; a CI assertion greps rendered manifests for any other StatefulSet/PVC-bearing workload and fails.
...
charts/plume/                    # umbrella
  ...
  charts/                        # vendored/pinned subcharts
    ...
    postgres                     # CNPG or bitnami — single instance core
    openobserve
...
- **Tiers (NFR-7)**: `tier: core | plus` — plus adds Argo Workflows, Phoenix, OpenFGA, eval runner images. Independently removable: `helm upgrade --set tier=core` cleanly drops plus components (no orphaned CRs — pre-delete hooks verify).
...
- **Contract compatibility gate**: the chart records the socket-contract versions it ships (`kgp/v1alpha1`, `idp/v1alpha1`, `receipt/v1`, semconv SHA); `helm upgrade` pre-hook compares against the running install and blocks a >1-version contract jump (the N/N−1 rule made operational).
- Rollback: `helm rollback` restores the engine; CRs/GitOps state are unaffected (the reconcile loop re-converges) — stated so operators trust it.
...
| Axis | Values |
|---|---|
| Distro | **k3d**, **kind** (required) · minikube weekly (scheduled) |
| Profile | local (both) · prod-shape on kind (replicas>1 where core allows) |
| Tier | core · core+plus (k3d only, time budget) |

Scenario per cell (the P1 demo, mechanized): install → deploy two sample agents from two SDK templates → A2A collaboration through the gateway → assert: receipts on the stream with correct principal chain; directory entries; SVIDs issued; `SandboxDowngraded` present (no gVisor in CI) and *only* that condition degraded; budget 429 on a driven overrun; teardown clean (no orphaned resources — namespace diff check). Plus the **budget-ledger check** (§2) and **stateful-dep grep** as separate fast jobs. Target wall-clock ≤ 20 min for the required cells (parallel).
