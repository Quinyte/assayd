...
- **Research**: Zitadel implements RFC 8693 token exchange natively (v2.49+; impersonation/delegation with audience-narrowing: exchanged tokens may never gain audiences absent from the inputs) + full management API for orgs/projects/clients/service accounts. https://zitadel.com/docs/guides/integrate/token-exchange
...
IdPProfile {
  ...
  ensureClient(spec): OAuth client {id, secretRef}        // kinds: app-login | external-agent | exposed-consumer | cli
...
Per install (and per tenant at design-26 time): ensure org (`default` in core), project `plume`, service accounts (agent-operator, gate-controller, receipt-tap), and standing clients (CLI device-code client; App login clients per App CR — provisioned by the operator at reconcile, not at bootstrap). All secrets land in k8s Secrets (ESO-compatible); nothing in CRs. Re-runs converge; deletions are never automatic (identity objects outlive CRs deliberately — finalizers only deactivate).
...
- The **gateway performs the exchange** (policy emitted by design 03), not agents — agents never hold user tokens; a compromised agent can't replay broader credentials.
...
- Agent↔agent (A2A) hops re-exchange per hop: the chain grows (`act` nesting), depth bounded by `maxHops` (design 22).
...
Profile-interface conformance fixtures run against **both** adapters (Zitadel testcontainer, Keycloak testcontainer) — the slot's N/N−1 discipline from day one. e2e (k3d): login → App → agent → tool with correct `act` chain in receipts; IdP-down drill (machine paths alive, human paths 401 + condition); exchange audience-narrowing negative test (agent must not reach a backend outside its compiled `aud`).
