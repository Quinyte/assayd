...
- **A2 (2026-08-20, landing 21-review R2-a)**: `ensureClient` carries a **`workflow-actor`** kind — the operator provisions one per Workflow at reconcile so a shared `workflow-runtime` still presents a per-workflow, gateway-visible principal for budgets (ADR-0020) and ReBAC checks (design 24). Same rationale as `agent-actor`: gateway-enforced promises need gateway-visible identities, and pod-attested SVIDs are never lent.
- **A3 (2026-09-03, from the critique of design 26 A1)**: A1's per-tenant SPIRE server inside a vCluster is **unmeasured and structurally in doubt**. spire-controller-manager adds a `k8s:pod-uid:<uid>` selector to every entry it renders, from the Pod it sees — inside a vCluster, the virtual Pod — while the host node's SPIRE agent attests against the host kubelet, which reports the host Pod's UID. Unless vCluster preserves Pod UIDs across its sync, which its documentation does not claim, no entry matches and no hard-mode workload is ever issued an SVID. A1 stands as the design; nothing may be built on it until the spike design 26 A1 owes has run. The fallbacks, if it fails, are a host-side controller-manager with a tenant-aware template or `workloadSelectorTemplates` mapping to host selectors, and either changes A1.
