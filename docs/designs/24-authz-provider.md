...
- **A1 (2026-09-03, from design 26 A1 and the cross-family review of design 02 A60) — the machine subject includes the trust domain.** A hard-mode tenant's SPIRE (design 26 §4, design 06 A1) issues `spiffe://<tenant-td>/agent/team-a/reviewer`, federated to the host; a soft-mode tenant's `reviewer` in `team-a` is issued in the host domain with the **same path**. §3's subjects are shaped `agent:pa-reviewer` — no namespace, no domain — so the ext-authz adapter would canonicalize both to one FGA subject, and the hard tenant would inherit the soft tenant's `can_invoke`/`can_call` tuples or poison a shared decision-cache entry. Federation makes the foreign SVID verifiable; only the mapping can make it distinguishable. So: the canonical machine subject is `agent:<trust-domain>/<agent-namespace>/<agent>`, every decision-cache and store-selection key includes the trust domain, and an SVID whose trust domain does not map to exactly one Tenant UID (or to the host) is **rejected**, never mapped by path. The operator-reconciled tuples of §4.1 are written with the same subject shape. **Owed**: a two-domain collision e2e with identical path components, and a sweep of every subject literal in this design and designs 03/04 to the new shape — not done here.
