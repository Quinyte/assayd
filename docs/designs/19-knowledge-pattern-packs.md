...
- **Ontology template** (per architecture Fig 8, now normalized to design 12 r2 grammar):
  ...
  - Relations (canonical triples): `"Procedure STARTS_AT Step"` (1..1) · `"Step NEXT Step"` (0..1) · `"Step NEXT Decision"` (0..1) · `"Decision BRANCH Step"` (attrs: {condition: str}, cardinality 2..*) · `"Step PERFORMED_BY Role"` · `"Step ON_EXCEPTION Step"`.
  ...
  - Bundles: `next_step` (recipe: walk; params: machine=Procedure, edge="Decision BRANCH Step", condition_attr=condition; require_scope: [Procedure, Step, Decision]) · `full_path` (recipe: subtree) · `who_does` (recipe: neighborhood_summary rooted at Role).
  - Probes: `generate: per_branch` (one per BRANCH edge, expected next step + citation) + template slots for 3–5 hand-written known-answer probes the wizard insists on ("write the questions your team actually asks — with the answer and the SOP section it comes from").
...
- **Starter labels**: 12 synthetic SOP snippets (claim intake, escalation, refund flows) with labeled Step/Decision/BRANCH extractions — the Gate A floor until the user's own ~20 labeled docs replace them (design 14 §4; the pack README says exactly this handoff).
