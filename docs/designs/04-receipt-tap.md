...
- **Status**: **approved** — critique PASS at r2 (reviews/04-review.md) · ADR-0021 · **A5 (2026-09-04) uncritiqued** — the witness fields A4 added have no producer, and `ns` attribution breaks under design 02 A42
...
```json
{
  ...
  "hop": {"type": "llm|mcp_tool|a2a|kg_query|http", "target": "…", "status": "ok|error|denied|budget_exceeded", "latency_ms": 412,
          ...
          "gatewayInstance": "…"},                                                                                     // A4 — which replica served
...
- **A5 (2026-09-04, from Codex r8 BLOCKER 13 against design 03 A43) — A4 added two fields and no producer, and two designs were already consuming them.** A4 argues correctly that `hop.target` is a non-injective string and that `hop.endpoint` and `hop.gatewayInstance` must exist. It then specifies neither: **no agentgateway OTLP attribute is named for either field, no plume-owned stamping mechanism is defined, and `gatewayInstance` has no grammar.** The only attribute-stamping clause in this design is §3.1's, and it is for `hop.type` — a different field, derived from "the backend class stamped into span attributes by design-03-emitted resources". Meanwhile design 03 A40/A43 and design 20 A2/A4 all read these fields as though they were specified, and design 03's fallback witness was defined over `hop.gatewayInstance` entirely, so `llmFallbackActive` could not legitimately have gone true.

  **Owed here, and design 03 has stopped waiting on it.** For `hop.endpoint`: either the exact `gen_ai.*` / agentgateway attributes that carry arm, instance fields and model — pinned to the semconv commit snapshot §3.1 already fixes — or, if the exporter does not emit them, a plume-owned stamping mechanism on the design-03-emitted resources, on the same footing as `hop.type`. For `hop.gatewayInstance`: the attribute or downward-API value that identifies the serving replica, **and its grammar**, since a Pod name and a Pod UID behave differently across a restart. Both need a primary-source research pass against v1.4.1 before anything is written down; A4's own owed conformance case — two same-arm, same-model endpoints producing distinguishable receipts — is the acceptance test for the first, and a second is owed for the instance identity surviving a rollout.

  **Until then the fields are absent, and the consumer says so rather than waiting**: design 03 A50 withdraws the replica-complete proof that rested on `gatewayInstance` — a per-replica count would measure load balancing rather than coverage even once the field exists, since nothing routes a canary to a chosen replica through a Service — and, because the canary attributes its result by `hop.endpoint`, which is equally absent, **design 03 claims no fallback remediation at all today, on any cluster**. Design 20 A7 carries the same withdrawal on the side that performs the activation.

  **The same gap covers `ns` and `tenant`, and it is load-bearing for attribution.** Both are envelope fields and `ns` is a subject segment, and no clause in this design states where the transform gets them. §4 calls the transform "stateless per batch" and its step list contains no Agent lookup, while §2 puts the tap inside the agent-operator binary, where a Kubernetes client is already in hand.

  Design 02 A42 makes this urgent rather than academic: an Agent's workload Pod now runs in `plume-run-<agent-namespace>`, so anything deriving `ns` from the Pod, the route or the backend records the **run** namespace — a namespace no Agent lives in. The consequence lands on the **audit index**, not the stream: §3.3's per-agent budget backstop and A2's rollup read the index, whose row set is `(receipt_id, task_id, tenant, agent, principal, ts, status, usd_est, pricing)` and carries **no `ns` column at all**. So two Agents of the same name in different namespaces already share one budget row today, before A42, and after A42 the namespace that would distinguish them is the wrong one. **Owed**: the authenticated attribute the transform reads, a rule that it is the Agent's **own** namespace and never its workload's, an index keyed `(tenant, ns, agent)` with `ns` in the row set, and the test that two same-named Agents in different namespaces keep independent budgets.
