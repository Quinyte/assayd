...
- **Path**: `GET /.well-known/agent-card.json` on the serving port, or the path in `spec.card.path`. It is only ever a path, and **the Agent CRD refuses anything else at apply time** (design 02 A76): it must start with `/`, use only the unreserved characters, the sub-delimiters, `:` and `@`, and be at most 1024 characters. So a `?`, `#` or `%` is refused — the operator builds the request URL from its parts and percent-encodes this value into the path, so a `?` would go out as `%3F` and `/card%20.json` as `/card%2520.json`, never as a query, a fragment, or the bytes the author wrote. What the rule does **not** refuse: dot segments (`/../../card.json` is admitted and sent literally) and a host-shaped path (`//elsewhere.example.com/card.json` is admitted). Neither moves the fetch — the operator writes the revision's own address first — but both `404`. An Agent stored before the rule keeps its path, under CRD validation ratcheting, and for that one the old behaviour still holds.
