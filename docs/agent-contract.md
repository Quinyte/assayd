...
| What the card path admits, what it refuses, that `path: ""` is refused, and that a stored one stays editable | `TestTheCardPathGrammarAndBoundAdmitWhatTheySay`, `TestMistakesAreRejectedWithActionableMessages`, `TestAnEmptyCardPathIsRefused` and `TestAStoredCardPathStaysEditable` in `test/envtest/` |
