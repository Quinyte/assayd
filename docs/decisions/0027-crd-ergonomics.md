...
**3. A binding resolves in the agent's own namespace, and there is no field to say otherwise.** Design 24 §4 derives the `can_call` ReBAC tuple *from* the binding, so a cross-namespace reference would authorize itself: anyone able to create an Agent in one namespace could reach a tool in another. A tool name may still be served by a Connector facet or an `MCPServer` — two kinds sharing one name space per namespace — which is why the flattening survives even though the original "one kind" premise did not. **Correction (2026-09-04): that uniqueness is enforced by nothing.** This ADR said "enforced at admission"; there is no such admission rule, no `MCPServer` CRD, and no design defines either the kind or the uniqueness rule. The flattening's *security* half — no `namespace` field, because a cross-namespace binding would authorize itself — stands on design 24 §4 and is unaffected. The *ergonomic* half — no kind discriminator — rests on a premise nothing yet holds up, and an amendment against design 11 is owed before it does. Design 02 §3.1 and §5 carry the same gap.
