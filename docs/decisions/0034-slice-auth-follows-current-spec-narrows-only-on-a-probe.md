...
- **Status**: accepted · 2026-09-11 · **supersedes ADR-0033** (including its Amendment 1). Decided by the human in three rounds: on `reviews/03-a52-recritique.md` (target, input, `Adopt`), on `reviews/03-a53-critique.md` (A1, B2, C2), and on `reviews/03-a54-critique.md` (D2, E2, F2). Design 03 A55 writes the last three in. · **Amended 2026-09-11 (Amendment 1)**: two clauses of D2 are re-attributed to design 03's author, and the human's G, H2 and I1 are recorded. · **Amended 2026-09-12 (Amendment 2)**: the human's J2 is recorded. · **Amended 2026-09-12 (Amendment 3)**: the human's decision that `Adopt` stays refused is recorded, with its reason, consent. · **Amended 2026-09-12 (Amendment 4)**: the human approved design 03's first slice and decided K2. · **Amended 2026-09-13 (Amendment 5)**: the human decided L1 for the Gateway-level auth gap, accepted its two costs, and decided W1. · **Amended 2026-09-15 (Amendment 6)**: the human decided that the `Lock` of a missing policy re-points its route at `status.activeRevision`, and that no `Lock` takes a probe answer until the Gateway reports on the route at its current generation — which also fixes a false credit open today in J2's and K2's `Lock`s. Design 03 A77 writes it in, and nothing implements it.
...
## Amendment 6 (2026-09-15, the human's decision on the missing-policy `Lock`'s route, and the costs accepted with it)

**The `Lock` of a missing policy re-points its route at `status.activeRevision`. The human decided it on 2026-09-15.** Amendment 4's approved slice gave J2's and K2's `Lock`s a route that follows promotion and gave the third `Lock` — the one that re-creates an `<agent>-auth` deleted out of band — a route it never writes. `cardAttributes` attributes a `401` on the card of the revision the route's `backendRef` names, so a served Agent whose route names a revision with no recorded card can never finish that `Lock`: it holds `PolicyApplyIncomplete` and `GovernanceSkipped` under `AuthPolicyMissing`, `Ready=False`, phase `Degraded`, and design 10 pages for as long as the Agent exists, while `status.activeRevision` advances past a revision receiving no traffic. Design 16's first slice documents the state and its exits, and its Q13 accepted it there on 2026-09-14, directing the fix here.

- **Decision.** At `ProbingAfter`, after the NACK read and after §3.3.2's route half holds, the `Lock` of a missing policy re-asserts `<agent>-serving` published on `status.activeRevision`, as J2's and K2's already do. Two conditions gate the write: `<agent>-auth` read present and equal to the target in that pass, and a recorded card digest for `status.activeRevision`. Design 03 A77 gives the rule.
- **And, for every `Lock`, a precondition of the probe.** No `Lock` takes a probe answer, or credits a `401`, until the assayd Gateway reports `Accepted` and `ResolvedRefs` on `<agent>-serving` **at the route's current generation**; until it does, the transaction returns to `Converging`. This is not a consequence of the first decision. **It fixes a defect in behaviour that already ships**: in J2's and K2's `Lock`s the route is re-asserted on `status.activeRevision` and probed in the same pass, so a promotion mid-transaction lets the previous revision's own `401` be credited to `<agent>-auth` against the new revision's card digest. A reproduction against `main` `3f37c25` measured it for both, and could not drive the missing-policy `Lock` there, which writes no route. A61 does not cover it: its clearing is gated on `beforeRevision` and touches the observed branch only.
- **What it costs. The human accepted all three costs on 2026-09-15.**
  - **A revision that never carried traffic through the route becomes reachable on it** while the `Lock` is short of `Served`. The route is open at that moment and nothing here claims otherwise: §3.3.2 proves control-plane convergence, never dataplane enforcement, and a NACK retains the previous configuration while every condition reports converged. The NACK read comes first so that a standing NACK breaks the pass before the write.
  - **One state still never finishes.** No card digest for the revision the route names and none for `status.activeRevision`. The conditions, the page and the exits are unchanged, and the exits are an administrator deleting `<agent>-serving` — an outage until `Create` republishes, unbounded while a Gateway-level or foreign policy holds that `Create` — or reverting the spec to the revision the route names, which does nothing for a card that never validates. The message must say the state is terminal and name both, which design 03's message does not do today.
  - **The re-point is conditional for a reason, and the condition has a price.** Without the recorded-digest condition, a route on a carded r1 beside an uncarded `status.activeRevision` would move from crediting to paging for ever — the inverse of the defect. With it, that Agent keeps finishing, and the Agent whose active revision is uncarded keeps a route naming a revision that receives nothing.
- **Rejected.** Re-pointing at `Converging`: the tuple has not held there, and the safety argument for the placement would be false. Keying the guard on "the `backendRef` changed in this pass": a status write lost to `Conflict`, a restart, or a route moved by someone else re-opens the window, and there is no status field to record the change in.

**Not the human's.** Design 03 A77 made these calls. They are recorded here so that this ADR is not read as deciding them:

- narrowing a `Lock`'s attribution to the revision the route's **single** `backendRef` names, so that the gate, the re-point and the attribution key on one object; a route not carrying exactly one is unattributable;
...
- the shape of §8.1 case 18, including the two sub-cases that pin the shipped J2 and K2 defect.
