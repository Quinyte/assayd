...
**The `Lock` of a missing policy re-points its route at `status.activeRevision`. The human decided that, and only that, on 2026-09-15.** Everything else below is design 03 A77's, made on 2026-09-16 in answering two critiques, and is recorded under "Not the human's" so that a later reader does not cite it as decided.
...
**Not the human's.** Design 03 A77 made these calls, on 2026-09-16, answering `reviews/03-a77-critique.md` and `reviews/03-a77-recritique.md`. They are recorded here because they change what the decision above means, and they are **not** the human's accepted costs:
...
- **A precondition of the probe, for every `Lock`**: no `Lock` takes a probe answer, or credits a `401`, until the assayd Gateway reports `Accepted` and `ResolvedRefs` on `<agent>-serving` at the route's current generation; until it does, the transaction returns to `Converging`. **This fixes a defect in behaviour that already ships.** In J2's and K2's `Lock`s the route is re-asserted on `status.activeRevision` and probed in the same pass, so a promotion mid-transaction lets the previous revision's own `401` be credited to `<agent>-auth` against the new revision's card digest. A reproduction against `main` `3f37c25` measured it for both, and could not drive the missing-policy `Lock` there, which writes no route. A61 does not cover it: its clearing is gated on `beforeRevision` and touches the observed branch only. The first draft keyed this on "the `backendRef` changed in this pass", which a lost or conflicted status write, a restart, or a route moved by someone else re-opens. *(First critique, MAJOR 2.)*
...
- **The terminal message.** Where the state above stands, the message says it is terminal until something outside the transaction changes, and names both exits; design 03's message does neither today. *(First critique, MAJOR 4.)*
...
- **The shape of §8.1 case 18**, including the two sub-cases that pin the shipped J2 and K2 defect and the one that pins the digest condition.
