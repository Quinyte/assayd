...
|---|---|---|---|
| A | Every `measured` claim's test exists, in the file it names. | A renamed or deleted test leaving a `measured` claim standing. | **Partly.** `check-claims.sh` requires `^func <test>(` somewhere under `test/`, `internal/` or `api/`, and requires the name to be a plain Go identifier so a pattern cannot certify itself. There is no file to check it against, and — §5.1 — it does not run when only the Go tree changes. |
...
| `/roadmap.html` | `docs/decisions/0030-*.md`; each design's own `- **Status**:` bullet; the badges on the page | the status table generated from the Status lines **once the Status-line fix below merges**; the gap list and frame hand-written | the status table: **owed** — the Status-line/index agreement test described below; the badges: CI (A and D, partly) |
...
| `/reference/crd-agent` | `api/v1alpha1/agent_types.go` → `config/crd/assayd.dev_agents.yaml`, via `cmd/refgen`, into `docs/reference/` | generated in the repository; **not yet ingested into the site** — see below the table | the Markdown: **CI** — `make verify` regenerates `docs/reference/` and fails if what is committed differs (PR #59). The site page: **owed** |
...
| `/reference/helm-values` | `charts/assayd/values.yaml`, via `cmd/refgen`, into `docs/reference/` | generated in the repository; not yet ingested | the Markdown: **CI**, the same `make verify` gate. The site page: **owed** |
...
- **Each design's own `- **Status**:` bullet — the holder for `/roadmap`'s status table, and for every `designed` claim's approval boundary.** `docs/designs/README.md`'s index summarises the Status lines; it is not a second holder. This is the human's decision of 2026-09-23, and it is recorded here as a decision rather than as something the corpus already agreed on, **because the corpus did not agree**: `AGENTS.md` and `CLAUDE.md` tell a reader to trust a design's Status line over any summary, while `README.md` says "`docs/designs/README.md` arbitrates", ADR-0030's own Consequences say "every design carries its own Status and `docs/designs/README.md` arbitrates", and `test/docs/superseded_test.go`'s `blanket-approval-claim` message ends "(ADR-0030; docs/designs/README.md arbitrates)". Those three read the other way from the decision, and reconciling them is the Status-line change's work, not this document's. An earlier revision of this document said `README.md`, `AGENTS.md` and the table "all say" the Status line wins; that was not true, and is withdrawn.
...
**The roadmap may read the Status lines — but not yet.** On `main` at `c4e5020`, eight designs' Status lines — 06, 07, 10, 12, 14, 20, 22 and 24 — open with **approved**, and that word is false for each of them: `docs/designs/README.md`'s index records every one of them as critique PASS and "awaiting user approval", and ADR-0030 rescoped the delivery commitment to one slice. A separate change, not merged at the time of writing, corrects those eight Status lines and adds a CI test that fails when a design's Status line and its index row disagree. **Until that change merges, nothing may generate an "approved" state from today's Status text**, because the generator would faithfully publish eight false approvals under the claim that it read the source of truth. After it merges, `/roadmap`'s status table reads the Status lines, and the new test is its protection; this document then owes a one-line update to the table above.

**The reference pages are generated into the repository, and are not on the site.** `cmd/refgen` writes `docs/reference/crd-agent.md`, `helm-values.md` and `conditions.md`, and `make verify` holds them to the code. **Nothing copies them into `web/docs/`**: `web/scripts/build.sh` runs the two Astro builds and nothing else, `web/docs/src/content/docs/` holds only the scaffold's two pages, and the Starlight sidebar (`web/docs/astro.config.mjs`) autogenerates one group, `guides`, and nothing else. **An ingestion step is owed, and its place is fixed**: it goes in `web/scripts/build.sh`, before the Astro build, copying `docs/reference/*.md` into the docs target's content directory, so that `web/scripts/check-links.sh` — which runs over the built `dist/` — checks every link in the copied pages. A copy made anywhere the link gate does not see would publish the generated pages' internal links unchecked, and those pages cite source files and sections by path. The sidebar entry is part of the same change.
...
One row per design, **quoting each design's own `- **Status**:` bullet verbatim rather than summarising it**. The Status line is the holder and `docs/designs/README.md`'s index is a summary of it — the human's decision of 2026-09-23, recorded in §5.4 with the disagreement in the corpus that preceded it.

**It is not generated yet, and must not be generated from today's text.** Eight Status lines on `main` at `c4e5020` open with **approved** where the design is not approved (§5.4 names them), so a generator reading them today would publish eight false approvals and cite the source of truth as its authority. A separate change corrects those lines and adds a CI test that fails when a Status line and its index row disagree. **The table is generated once that change is on `main`**, and that test is what protects it; until then the roadmap carries no design status table at all, rather than a hand-written one that would be a third copy.
...
| 1 | **The claim component and its check — built, in part, by PR #57.** `ClaimClass` with inline evidence, `resolveClaim`'s build-time throw, `web/scripts/check-claims.sh` over the built HTML, `web/scripts/block_scope.py`, and `/claim-classes.html`, all run from `.github/workflows/web.yml`. **What remains of this item, in order**: (a) run `check-claims.sh` when the Go tree or `docs/designs/` changes, not only `web/**` (§5.1); (b) read the section in gate D; (c) fixtures that fail each check, gate G; (d) the by-path page list, gate H. Gates B, C, E and F are not built and wait for the manifest (§5.2). | Infrastructure, not a page. Every badge on every page depends on it. (a) comes first because without it the check that exists is blind on exactly the change that breaks it. |
...
| 6 | `assayd.io/roadmap.html` | Needs items 1 and 3. Its §8.3 gap list is the reason the disclosure posture was decided, so it should not wait. **Its design status table waits for the Status-line correction to merge (§8.2)**; the page ships without that table rather than with a hand-written one. |
...
### 11.2 Counts and cross-references — five fixed, one live again
...
| 6 | "design 02 carries eleven (A1–A11)" | **Live again.** PR #60 corrected it in §20's *Process note* to "**seventy-six**, A1–A76, the tip dated 2026-09-16". Design 02 A77 (PR #54, 2026-09-23) has since added a seventy-seventh, so the corrected count is stale. A fix is in progress in a separate change. **The lesson for the site is the row itself**: a hand-typed count of a growing set is stale on the next amendment, which is why §3.2 cites `docs/reference/conditions.md` for reason counts rather than copying one. |
...
- **§11.2 row 6**, design 02's amendment count, stale since A77 and being fixed separately.
