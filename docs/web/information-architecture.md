...
- **Status**: **proposal, not approved.** No critique of this document has been run. It specifies pages, not prose. When it was written no page it names existed; PR #67 built three — `/concepts/the-auth-transaction.html` on `.io`, and `/what-runs-today.html` and the three generated `/reference/*.html` pages on `.dev` — and the tables below say so where each is listed. Where it proposes a file outside `docs/web/`, that file is owed by whoever implements the section, and this document does not create it.
...
| `/what-runs-today.html` | After `helm install`, what is actually in my cluster? — **built by PR #67**, which this table did not originally list |
...
| `/what-runs-today` | `charts/assayd/` rendered; the badges on the page | hand-written, with inline claims | CI (A and D, partly) per badge; and `TestADefaultInstallRunsOneOperatorAndNothingElse`, run by `ci.yml`'s chart job, holds the chart's exact rendered inventory AND fails if the page stops naming its policies, its Deployment and its replica counts (PR #67) |
...
| `/reference/conditions` | `api/v1alpha1` and `internal/controller`, read with `go/types` by `cmd/refgen`; the three per-reason annotations that are not in the code live in `internal/refgen/reasons.yaml` | generated in the repository; copied into the site (PR #67) | the Markdown: **CI** — `make verify`, plus `TestNoConditionTypeIsALiteral`. The join pins the *set* of reasons; it does not prove the annotations are still true of the code, and the page says so. The site page: **CI**, as above |
