...
- **Re-verify:** at any change to `runLock`'s `ProbingAfter` arm, to `cardAttributes`, or to `ensureServingRoute`'s route write. **Closed by PR #48**, which implemented design 03 A77: the scratch fixture below was re-run against that branch first and printed the defect verbatim, and under the change both reproductions stop at `ProbingAfter` with nothing credited. What pins it for good is design 03 §8.1 case 18, in `test/envtest/authgate_test.go`.
...
- **§3.3.3's attribution is sound as a rule and unsound as implemented**, for J2 and K2. A77 is the fix — the route gate for every `Lock`, and the re-point for the missing-policy `Lock` — and **PR #48 implements it**. §8.1 case 18 (f) and (g) are the tests for the shipped defect, and (d) and (h) for the gate; all eight are written, and their mutations are tabled in design 03 A77.
