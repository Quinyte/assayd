...
- **NFR-1 Weight**: core control plane ≤ 8 pods. **Enforced** by `TestCorePodBudget` (`test/chart/chart_test.go`), which sums `spec.replicas` over the rendered Deployments and StatefulSets, adds one per DaemonSet, and compares against `const budget = 8`; raising the budget means editing that constant in the same commit, which is what puts the change in front of a reviewer. **The ADR itself is a convention, not a check** — nothing verifies that one was written, and NFR-1's "every added pod requires a written justification in an ADR" is enforced by review discipline alone. The second half — *exactly one pod of assayd-authored code in core* — is **not enforced, and it is false at the default profile**. `charts/assayd/values.yaml` sets `operator.replicas: 2` under the default `profile: prod`, so a default install runs **two** assayd-authored pods, one of them a never-Ready standby holding the leader lease. The test prints the disproof and asserts nothing about it: `core pod count: 2/8 — map[assayd-agent-operator:2]`. An earlier version of this correction said "one workload", which is true and is not what the requirement claims — the requirement counts **pods**. Either the requirement means one *workload* and should say so, or `profile: local` (which collapses replicas to 1) is the only profile that meets it. Architecture §17 carries the target ledger.
- **NFR-2 Stateful deps**: **the enforced rule is an allowlist of three, not a pair.** `TestStatefulDependencyAllowlist` (`test/chart/chart_test.go`) refuses any rendered `StatefulSet` or `PersistentVolumeClaim` whose name does not **contain** `postgres`, `nats` or `openobserve` — a substring match, so `nats` also admits a hypothetical `nats-sidecar-cache`; the third entry is recorded in the test as "observability sink, not substrate". The original wording was *"Postgres and NATS JetStream are the only stateful dependencies, ever"*, and it is kept here because the gap is the point: **"ever" and a three-entry allowlist cannot both stand.** The distinction the allowlist draws — substrate versus sink — may well be the right one; it has never been written down as a decision, so the requirement and the test have disagreed in silence. Anything else that wants a disk has to be named in the allowlist, in the change that introduces it. **How well that works is untested**: the chart renders **zero** StatefulSets and **zero** PersistentVolumeClaims today, so the check has never had anything to refuse and passes vacuously. It will start doing real work the first time the chart deploys its own substrate.
