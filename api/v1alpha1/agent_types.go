...
// +kubebuilder:validation:XValidation:rule="!has(self.env) || self.env.all(e, !e.name.startsWith('PLUME_'))",message="spec.runtime.env may not set a name beginning with PLUME_. That prefix is the operator's injected contract (design 02 §11) — PLUME_GATEWAY_URL and its siblings — and a container keeps the LAST duplicate, so setting one here would redirect the agent away from the gateway every budget, tool grant and egress rule is enforced at. Choose another name. See ADR-0031 and design 02 A65."
// +kubebuilder:validation:XValidation:rule="!has(self.envFrom) || self.envFrom.all(f, !has(f.prefix) || !f.prefix.startsWith('PLUME_'))",message="spec.runtime.envFrom may not use a prefix beginning with PLUME_: it is the operator's injected contract, and a ConfigMap or Secret mapped under it could shadow PLUME_GATEWAY_URL. See design 02 A65."
