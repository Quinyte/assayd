...
**And an MCP tool call goes through it.** `TestAnAgentCallsAnMCPToolThroughTheGateway` runs a real MCP exchange — `initialize`, `tools/list`, `tools/call` — against `test/mcpserver`, a two-tool Streamable HTTP server behind an `AgentgatewayBackend` on a second Gateway listener. A hand-authored `backend.mcp.authorization` allowlist (design 03's `toolAllowlist`) does more than refuse: it **removes the disallowed tool from `tools/list`**, so an agent never learns it exists, and a refused call returns `Unknown tool` rather than "forbidden" — good for non-disclosure, a trap for whoever debugs it. Flipping the rule inverts the listing and both calls. **And the A2A half is literal too.** `test/responder` implements `SendMessage` on the HTTP+JSON binding its card declares, and `TestAnAgentAnswersThroughTheGateway` sends a `SendMessageRequest` through the operator's route and requires a `TASK_STATE_COMPLETED` task back — so the A2A half of ADR-0030's clause holds for a client — not yet for an agent: the MCP call is made by the test harness, not by the responder, and the two are separate tests against separate backends, so "one **agent** completes one A2A task and one MCP tool call" is still not literal. It is ONE method, on A2A 1.0 only, on a test fixture: nothing else in A2A is implemented, the operator speaks none of it, and the A2A client is design 08's and does not exist (design 07 A6.12). The next work is the rest of the narrow slice in ADR-0030, not the next design.
